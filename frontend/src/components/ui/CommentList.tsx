...
const CommentList: React.FC<CommentListProps> = ({ reviewId }) => {
  ...
  // 댓글 폼 훅 사용
  const { formData, handleChange, handleSubmit, isSubmitting, submitError, submitSuccess } =
  ...
  const handleEditComment = async (commentId: number) => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/comment/update`,
  ...
  const handleDeleteComment = async (commentId: number) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/comment/${commentId}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      refetch();
    } catch {
