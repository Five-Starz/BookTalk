...
const ReviewCard = ({review}: ReviewCardProps ) => {
  ...
  useEffect(() => {
    const checkLiked = async () => {
      if (!userId) return;
      const res = await fetch("https://booktalk-server.shop/likes/find", {
        method: "POST",
  ...
  const handleLike = async () => {
    ...
    if (isLiked) {
      // 해제
      await fetch("https://booktalk-server.shop/likes/del", {
        method: "POST",
      ...
      // 등록
      await fetch("https://booktalk-server.shop/likes", {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/x-www-form-urlencoded" },
        body: `reviewId=${review.reviewId}`,
      });
