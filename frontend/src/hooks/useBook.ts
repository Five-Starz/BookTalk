  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      ...
      try {
        setIsLoading(true)
        setError(null);
        const bookmarkcount = await axios.post(`http://35.216.79.174:3000/bookmarks/count`, {
          isbn: isbn
        });
        const bookrating = `http://35.216.79.174:3000/books/averageRating/${isbn}`;
...
export const useBookDetailsInMyPage = (isbn: string | undefined): UseBookDetailsResult => {
  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      ...
      try {
        const response = await axios.get<BookDetail>(`http://35.216.79.174:3000/books/info/${isbn}`);
  ...
  useEffect(() => {
    ...
    const fetchRecommendList = async () => {
      try {
        ...
        const response = await axios.get<BookDetail[]>(`http://35.216.79.174:3000/books/random`);
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        ...
        const requestUrl4 = `http://35.216.79.174:3000/reviews/search2/{isbn}?isbn=${isbn}`;
