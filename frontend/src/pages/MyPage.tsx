...
const MyPage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // 1. 유저 인증 정보 가져오기.
        const authRes = await axios.get('https://booktalk-server.shop/auth/protected', {
          headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` }
        });
        const { userId } = authRes.data.user;

        // 2. 최신 프로필(닉네임 등)은 별도 fetch
        const profileRes = await axios.get(`https://booktalk-server.shop/auth/${userId}`);
        ...
        // 2. 리뷰 수 가져오기
        const reviews = await axios.get(`https://booktalk-server.shop/reviews/count/${userId}`);
        setReviewCount(reviews.data);

        // 3. 보고싶어요 수 가져오기
        const bookmarks = await axios.get(`https://booktalk-server.shop/bookmarks/${userId}`);
...
export const ReviewCollection = () => {
  ...
  useEffect(() => {
    ...
    const fetchReviewData = async () => {
      ...
      try {
        // 1. 리뷰 목록 불러오기
        const res = await axios.get(`https://booktalk-server.shop/reviews/user/${userId}`);
        ...
        const reviewWithExtras = await Promise.all(
          reviews.map(async (review: Review) => {
            ...
            try {
              // 실제 API 경로/응답에 맞게 수정!
              const bookRes = await axios.get(`https://booktalk-server.shop/books/search?query=${review.isbn}`);
            ...
            let likeCount = 0;
            try {
              const likeRes = await axios.post(`https://booktalk-server.shop/likes/count`, { reviewId: review.reviewId });
            ...
            let commentCount = 0;
            try {
              const commentRes = await axios.get(`https://booktalk-server.shop/comment/review/count/${review.reviewId}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`https://booktalk-server.shop/reviews/${targetReviewId}`, {
        headers: {
...
export const WantReadList = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookmarkData = async () => {
      try {
        // 1. 북마크(보고싶어요) 리스트 호출
        const res = await axios.get(`https://booktalk-server.shop/bookmarks/${userId}`);
        ...
        const bookDetails = await Promise.all(
          bookmarkList.map(async (item: BookItem) => {
            try {
              // Book API로 도서 정보 조회
              const bookRes = await axios.get(`https://booktalk-server.shop/books/search?query=${item.isbn}`);
...
export const Settings = () => {
  ...
  const onValid = async (data: UpdateFormData) => {
    ...
    try {
      ...
      await axios.post(
        "https://booktalk-server.shop/auth/passupdate",
        sendData,
  ...
  const handleConfirmResign = async () => {
    ...
    try {
      await axios.delete(`https://booktalk-server.shop/auth/del/${userId}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      clearTokens();
      clearUser();
