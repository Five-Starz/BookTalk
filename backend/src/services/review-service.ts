...
class ReviewService {
  ...
  // 2. 특정 책의 전체 리뷰 조회
  async searchReviewsByBook(data: { isbn: string }) {
  ...
  // 3.리뷰 수정
  async updateReview(data: { reviewId: number; rating: number; content: string; userId: number }) {
    // 0. 이미 존재하는 리뷰인지 체크
  ...
  // 4. 리뷰 삭제
  async deleteReview(data: { reviewId: number; userId: number }) {
    // 0. 이미 존재하는 리뷰인지 체크
    const existing = await reviewRepository.hasReviewByUser(data.reviewId, data.userId);
    if (!existing) {
      throw new Error('[review-service error] 사용자가 해당 책에 작성한 리뷰가 존재하지 않습니다.');
    ...
    if (reviewId == undefined) {
      throw new Error(
        '[review-service error] 사용자가 해당 책에 작성한 reviewId를 가져올 수 없습니다.'
      );
    }
