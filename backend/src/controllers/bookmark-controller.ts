...
export class BookmakrsController {
  ...
  async findById(req: Request, res: Response, next: NextFunction): Promise<any> {
    ...
    if (userBookmarks.length > 0) return res.status(200).json(userBookmarks);
    else return res.status(404).json({ message: '해당 유저의 북마크가 존재하지 않습니다.' });
  }
  ...
  async delete(req: Request, res: Response, next: NextFunction) {
    ...
    if (del) res.status(200).json({ message: '삭제성공' });
    else res.status(400).json({ message: '삭제실패' });
  }
